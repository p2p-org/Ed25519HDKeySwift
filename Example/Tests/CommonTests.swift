...
class CommonTests: XCTestCase {
    ...
    // MARK: - Vector2
    let vector2Seed = "fffcf9f6f3f0edeae7e4e1dedbd8d5d2cfccc9c6c3c0bdbab7b4b1aeaba8a5a29f9c999693908d8a8784817e7b7875726f6c696663605d5a5754514e4b484542"
    let vector2: [[String: String]] = [
        [
            "path": "m/0'",
            "chainCode":
    ...
    func testGetMasterKeyFromSeed() throws {
        ...
        // vector2
        let keys2 = try Ed25519HDKey.getMasterKeyFromSeed(vector2Seed)
        XCTAssertEqual(keys2.key.toHexString(), "171cb88b1b3c1db25add599712e36245d75bc65a1a5c9e18d76f9f2b1eab4012")
        XCTAssertEqual(keys2.chainCode.toHexString(), "ef70a74db9c3a5af931b5fe73ed8e1a53464133654fd55e7a66f8570b8e33c3b")
    }
    
    func testPathDerivation() throws {
        try vector1.forEach { vector in
            let keys = try Ed25519HDKey.derivePath(vector["path"]!, seed: vector1Seed)
            let publicKey = try Ed25519HDKey.getPublicKey(privateKey: keys.key)
            
            XCTAssertEqual(keys.key.toHexString(), vector["key"])
            XCTAssertEqual(keys.chainCode.toHexString(), vector["chainCode"])
            XCTAssertEqual(publicKey.toHexString(), vector["publicKey"])
        }
        
        try vector2.forEach { vector in
            let keys = try Ed25519HDKey.derivePath(vector["path"]!, seed: vector2Seed)
            let publicKey = try Ed25519HDKey.getPublicKey(privateKey: keys.key)
